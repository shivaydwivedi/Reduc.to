...
Phase 4 implementation status: UUID v7 values are generated by `apps/api/src/shared/ids/uuid-v7.ts` and stored in PostgreSQL `uuid` columns through Prisma. Endpoint behavior that returns IDs is still deferred until product routes exist.
